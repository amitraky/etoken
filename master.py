...
def back_send_to_client(fileUrl,fileName):
    print("calling cb....")
    url = 'http://localhost:8000/api/save_sigend_certificate_cb?file_name='+fileName
...
class Signer(Singers):   
    def __init__(self,password):
        ...
        print(token)
        dico = token.to_dict()       
        lable = dico.get('label')
        lable = lable.strip()
        #lable =  "ePass2003"
        #print(len(lable))
        self.lable = str(lable)
        self.name = ''
        print("Lable Name top ",self.lable)
        self.certificate()
    ...
    def certificate(self):           
        ...
        try:
            ...
            for pk11object in pk11objects:
                ...
                print("Owner Name : ",owner_full_name)           
    ...
    def sign(self, keyid, data, mech):
        print("Signing ! ", "ePass2003", self.password)
        self.login(self.lable, self.password)
...
def upload():
    ...
    pdfUrl = "http://localhost:8000/certificate"+str(setyear.year)+"/"+fileUrl
    print(pdfUrl)
